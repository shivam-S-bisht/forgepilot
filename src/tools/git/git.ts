...
/**
 * Strip unwanted trailers (e.g. "Made by Cursor") from commit messages on a
 * sub-agent branch that are ahead of mainBranch.
 * Uses `git rebase --exec` with sed to clean each commit in one pass.
 */
export async function stripCommitTrailers(
	...
	const TRAILER_RE = /(?:Made by Cursor|Generated by |Co-authored-by:.*[Cc]ursor|Signed-off-by:.*[Cc]ursor)/;
...
export async function mergeSubAgentBranches(
	...
	for (const analysis of branchAnalyses) {
		...
		// Strip "Made by Cursor" and similar trailers before merging
		await stripCommitTrailers(repoPath, analysis.subBranch, mainBranch);
		// Return to main branch after strip (it checks out the sub branch)
		await gitExec(repoPath, ['checkout', mainBranch]);
